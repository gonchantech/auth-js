...
import {
  ...
  authClientWithSession2 as authWithSession2,
...
describe('Signout behaviour', () => {
  ...
  test('signOut should remove session if user is already signed out globally from another device', async () => {
    const { email, password } = mockUserCredentials()
    ...
    const { data: sameUser, error: signIn2Error } = await authWithSession2.signInWithPassword({
      email,
      password,
    })
    expect(signIn2Error).toBe(null)
    expect(sameUser).not.toBe(null)

    // Sign out globally from the first device
    const { error } = await authWithSession.signOut()
    expect(error).toBe(null)

    // Session remains on the second device but is invalidated on the server
    const {
      data: { session: session2BeforeSignOut },
      error: session2BeforeSignOutError,
    } = await authWithSession2.getSession()
    expect(session2BeforeSignOut).not.toBe(null)
    expect(session2BeforeSignOutError).toBe(null)

    // Should remove the session from the second device without throwing an error
    const { error: secondSignOutError } = await authWithSession2.signOut()
    expect(secondSignOutError).toBe(null)

    const {
      data: { session: session2AfterSignOut },
      error: session2AfterSignOutError,
    } = await authWithSession2.getSession()
    expect(session2AfterSignOut).toBe(null)
    expect(session2AfterSignOutError).toBe(null)
  })
})
